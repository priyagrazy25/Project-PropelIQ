...
public sealed class DeactivateUserCommandHandlerTests
{
    ...
    private static (DeactivateUserCommandHandler handler, Mock<IIdentityDbContext> dbMock)
        CreateHandler(List<User>? users = null)
    {
        ...
        var logger = new Mock<ILogger<DeactivateUserCommandHandler>>();
        var handler = new DeactivateUserCommandHandler(dbMock.Object, logger.Object);

        return (handler, dbMock);
    }

    [Fact]
    public async Task HandleAsync_ValidDeactivation_SetsStatusInactive()
    {
        var user = new User
        {
        ...
        var result = await handler.HandleAsync(new DeactivateUserCommand(TargetId, AdminId));
    ...
    [Fact]
    public async Task HandleAsync_SelfDeactivation_ReturnsFailure()
    {
        var (handler, _) = CreateHandler();

        var result = await handler.HandleAsync(new DeactivateUserCommand(AdminId, AdminId));

        Assert.False(result.IsSuccess);
        Assert.Equal("SELF_DEACTIVATION", result.Error);
    }
    ...
    [Fact]
    public async Task HandleAsync_AlreadyInactive_ReturnsFailure()
    {
        var user = new User
        {
        ...
        Assert.False(result.IsSuccess);
        Assert.Equal("ALREADY_INACTIVE", result.Error);
    }
}
